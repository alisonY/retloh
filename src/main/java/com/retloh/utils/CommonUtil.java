...
public class CommonUtil {
	...
	public static String listToString(List<String> list, String separator) {
		...
		for (String description : list) {
			descriptions.append(description).append(separator);
		}
     ...
     * 字符串加密(部分隐藏)
     * @param input	待加密字符串
     * @param start	索引之前明文显示
     * @param end	索引之后明文显示
     * @param flag  加密字符
     * @return		加密后的字符串
     */
    public static String replaceByIndexRange(String input,int start,int end,String flag)
    {
    	int len = input.length();
    	String startStr = input.substring(0, start);
    	String endStr = input.substring(len-end, len);
